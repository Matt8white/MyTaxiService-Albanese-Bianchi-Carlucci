...
\paragraph{Structure} Here we choose to use the \emph{Object-Oriented Architectural Style}\cite{ooa}.
...
\toprule
    ...
    \midrule
    \multirow{2}{*}{signup} 			& 	\multirow{2}{*}{NO}		&	\multirow{2}{*}{Guest}	&	Username			&	Username for this customer			& - \\
    									&							&							& Password			& Password for this customer \\\cmidrule{1-6}
    ...
    \multirow{2}{*}{confirmEmail} 			& 	\multirow{2}{*}{YES}		&	\multirow{2}{*}{Customer}	&	Username			&	Username chosen during signup 			& Boolean response \\
    									&							&							& Mail		& Mail checked \\\cmidrule{1-6}
...
which is the one we have chosen, \emph{Push Notification}.

We decided to support both iOS and Android smartphones. In order to send push
notifications to iOS smartphone, we have to use \emph{Apple Push Notification Service}\cite{apns}.
The same applies to Android world, Google has its own push service, called 
\emph{Google Cloud Messaging}\cite{gcm}.
